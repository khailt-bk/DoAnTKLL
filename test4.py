...
img = cv2.imread('Test4.png')                   #doc vao hinh anh
...
l_th = 700                                     #low threshold cua filter Canny
u_th = l_th + 50                        #high threshold cua filter Canny
edges = cv2.Canny(gray, l_th, u_th, apertureSize=3)  # filter phat hien canh
# cv2.imshow('ed', edges)
# cv2.waitKey(0)
lines = cv2.HoughLines(edges, 1, np.pi / 180, 77) # thuat toan Hough Line Transform
